"""
high_level_agent.py  (v3 — registry-driven + MLflow)
"""
...
from shader_registry import (
    MODIFIER_NAMES, FEEDBACK_SOURCE_NAMES, version_hash,
    N_MODIFIERS,
)

# ─────────────────────────────────────────────────────────────────────────────
#  Goal definitions — reference modifier names from registry, not hardcoded
# ─────────────────────────────────────────────────────────────────────────────

# Preferred shaders are filtered to only those that exist in the registry
def _filter(names: list[str]) -> list[str]:
    return [n for n in names if n in MODIFIER_NAMES or n in FEEDBACK_SOURCE_NAMES]
...
GOAL_DEFS = {
    ...
    CALM:        {"target_intensity": 0.1,  "preferred_shaders": _filter(["color_grade"]),                "transition": False},
    MEDIUM:      {"target_intensity": 0.4,  "preferred_shaders": _filter(["color_grade","bloom"]),        "transition": False},
    HIGH_ENERGY: {"target_intensity": 0.9,  "preferred_shaders": _filter(["glitch","chromatic","bloom"]), "transition": False},
    TRANSITION:  {"target_intensity": 0.5,  "preferred_shaders": _filter(["color_grade"]),                "transition": True},
    GLITCH_MODE: {"target_intensity": 0.8,  "preferred_shaders": _filter(["glitch","chromatic","feedback"]), "transition": False},
    AMBIENT_MODE:{"target_intensity": 0.3,  "preferred_shaders": _filter(["kaleidoscope","feedback","color_grade"]), "transition": False},
}
...
class HighLevelAgent:
    def __init__(
        ...
        self.trainer = PPOTrainer(
            visual_dim    = HIGH_LEVEL_DIM,
            audio_dim     = AUDIO_FEAT_DIM,
            n_actions     = N_HL_ACTIONS,
        ...
        if not self.trainer.load(versioned_path):
            print(f"[HighLevel] No checkpoint at {versioned_path}, starting fresh.")
        ...
        self.metrics.log_params({
            "hl_rollout_steps":  rollout_steps,
            "hl_step_interval":  step_interval,
            "hl_n_actions":      N_HL_ACTIONS,
    ...
    def observe(self, state: EngineState, audio_features: AudioFeatures = None) -> None:
        self.history.push(state)
        if audio_features is not None:
            self.metrics.log_audio(audio_features)

    @staticmethod
    def _audio_to_array(af: AudioFeatures | None) -> np.ndarray:
        if af is None:
    ...
    def step(self, state: EngineState, audio_features: AudioFeatures = None) -> dict:
        ...
        if self.step_n > 0:
            ...
            self.metrics.log_high_reward(comp, reward)
        ...
        goal_name = GOAL_NAMES.get(action, "UNKNOWN")
        self.metrics.log_goal(goal_name)
        self.metrics.tick_high()
